...
func _on_month_year_confirm_pressed():
	year = int(current_year_label.text)
	var temp_selected = -1
...
func _on_left_year_pressed() -> void:
	current_year_label.text = str(int(current_year_label.text) - 1)


func _on_right_year_pressed() -> void:
	current_year_label.text = str(int(current_year_label.text) + 1)
